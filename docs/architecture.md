...
- **지수 RSI 다이버전스 천장경고** — ^GSPC/^IXIC의 RSI bearish divergence(HH 가격 vs LH RSI)를 5-bar fractal 스윙·ZigZag 최소 스윙폭·120거래일 룩백 게이트로 탐지해 append-only 로그에 보존. 1999~2026 백필 검증 precision 37.5% vs 기저율 15.5%(lift 2.42배)로 채택 문턱 상회. 하락장 선행 감지기와 마찬가지로 **관찰·로깅 전용** — 판정·PF 연결은 별도 검증 후
...
- 격상 게이트가 깨지면 강등
- **청산 룰**: Hard Stop(당일 종가 ≤ 진입가 × (1 − 2×ATR%)) → Phase 3 overstay(4주) 순. 청산선은 편입 시점에 결정론적으로 박제(`hard_stop_price`)되어 사이징의 손절폭과 정합
...
- 리포트 발행 (일간 / 주간 시장 · 주간 종목 2건 분리 / 기업 / QA)
- QA 게이트 3축 채점
...
| 분석·추천 | tracked_stocks, portfolio_positions, signal_log, tracked_forward_returns | 에이전트 산출물 + 고정-호라이즌 팩터 검증 |
| 토론·학습 | debate_sessions, theses, agent_learnings, narrative_chains, narrative_chain_research_runs, market_risk_scores, index_rsi_divergence_signals, macro_indicators | 토론 + 딥리서치 + 학습 + 리스크 감지 |
...
| 백테스트 | bt_daily_rs, bt_stock_phases, bt_exit_validation_runs, bt_daily_atr | 생존편향 해소 시뮬레이션 (운영 ETL과 격리) |
