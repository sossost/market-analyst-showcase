...
```
[GitHub] triaged 라벨이 붙은 이슈
        ↓
[Cron 매 정시] Issue Processor 기동
        ↓
[Filter] in-progress 라벨 없는 이슈 1건 선택
        ↓
[Claude Code CLI 호출]
  ├─ 이슈 본문 + 라벨 + 첨부 PRD 분석
  ├─ 워크트리 생성 (병렬 세션 충돌 방지)
  ...
  └─ Conventional Commit 메시지로 커밋
        ↓
[gh pr create] PR 생성 + 라벨 부여
        ↓
[Discord] PR URL + 요약 알림
```
...
```
[Cron 매 :30분] PR Reviewer 기동
        ↓
[gh pr list --state=open]
        ↓
[각 PR마다 병렬 실행]
  ├─ Strategic Review
  │   └─ 회사 골(알파 생성)과의 정렬도 평가
  │   └─ 도메인 경계 침범 여부
  │   └─ 기존 patterns/decisions와 충돌 여부
  └─ Code Review
      └─ 보안, 정확성, 가독성, 테스트 커버리지
...
```
strategic-aide 에이전트 실행
        ↓
다음 항목 자동 분석:
  ├─ 회사 골(알파 생성) 대비 현재 거리
  ├─ tracked_stocks 분포 / 누적 성과
  ├─ 활성 thesis 현황 + 적중률
  ├─ 최근 1주일 운영 이상치
  └─ 미해결 이슈 우선순위
        ↓
memory/strategic-briefing.md 갱신
```
